...
def controller(observation, es, ex, ex_int, s_d, dt, t):
    ################################
    # quat -> rpy
    quat = observation[3:]
    rotmat_WB = np.array([
        [1 - 2 * (quat[2] ** 2 + quat[3] ** 2), 2 * (quat[1] * quat[2] - quat[3] * quat[0]),
         2 * (quat[1] * quat[3] + quat[2] * quat[0])],
        [2 * (quat[1] * quat[2] + quat[3] * quat[0]), 1 - 2 * (quat[1] ** 2 + quat[3] ** 2),
...
def main():
    ...
    for t in range(1000):
        ...
        F = controller(observation, es=es, ex=ex, ex_int=ex_int, dt=dt, s_d=s_d, t=t)
